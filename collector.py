...
#Only implemented for one door in prototype
###
...
class SampleApp(tk.Tk):
    ...
    def demo(self):
        # Demo the system by scanning legi to open/close door
        while True:
            global scanned_item
            scanned_item = []
            self.scan()
    ...
    def door1(event):
        # Opens and closes door to enable drop-off of samle    
        motordoor1.enable(True)        # enables stepper driver
        motordoor1.run(4500, True)     # run motor steps clowckwise
        sleep(10)
        motordoor1.run(4500, False)    # run motor steps counterclockwise
        motordoor1.enable(False)       # disable stepper driver

    def motor1_forward(event):
        # Enable motor and run motor script        
        motordoor1.enable(True)        # enables stepper driver
        motordoor1.run(2000, True)     # run motor 2000 steps clowckwise
        motordoor1.enable(False)       # disable stepper driver
    
    def motor1_backward(event):
        # Enable motor and run motor script        
        motordoor1.enable(True)        # enables stepper driver
        motordoor1.run(2000, False)    # run motor 2000 steps counterclockwise
        motordoor1.enable(False)       # disable stepper driver
